...
public class Computer {
	...
	public Computer(Player first, Player second) {
		firstPlayer = first;
		secondPlayer = second;
	...
	public void move(Direction d) throws NotEnoughResourcesException, UnallowedMovementException {
		if (this.getSecondPlayer().getTeam().contains(this.getCurrentChampion())) {
		...
		if (this.getSecondPlayer().getTeam().contains(this.getCurrentChampion())) {
			if (newx < 0 || newx >= BOARDHEIGHT || newy < 0 || newy >= BOARDWIDTH)
				return;
		} else if (newx < 0 || newx >= BOARDHEIGHT || newy < 0 || newy >= BOARDWIDTH)
	...
	public void attack(Direction d)
		...
		if (this.getSecondPlayer().getTeam().contains(this.getCurrentChampion())) {
			if (hasEffect(getCurrentChampion(), "Disarm") || getCurrentChampion().getCurrentActionPoints() < 2)
				return;
		} else {
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					((Cover) board[currx][curry]).animation.attacked = true;
					((Cover) board[currx][curry]).animation.currentHp = curhp;
					((Cover) board[currx][curry]).setCurrentHP(curhp);
					if (curhp <= 0) {
						((Cover) board[currx][curry]).animation.direction = "dead";
						board[currx][curry] = null;
					}
					return;
					...
					target.animation.attacked = true;
					target.animation.currentHp = target.getCurrentHP();
					if (target.getCurrentHP() <= 0) {
						target.animation.direction = "dead";
	...
	private void validateCastAbility(Ability a) throws NotEnoughResourcesException, AbilityUseException {
		if (this.getSecondPlayer().getTeam().contains(this.getCurrentChampion())) {
			if (getCurrentChampion().getMana() < a.getManaCost()
					...
					|| hasEffect(getCurrentChampion(), "Silence") || a.getCurrentCooldown() > 0)
				return;
		} else {
			if (getCurrentChampion().getMana() < a.getManaCost())
	...
	public void castAbility(Ability a, int x, int y) throws NotEnoughResourcesException, AbilityUseException,
		...
		if (this.getSecondPlayer().getTeam().contains(this.getCurrentChampion())) {
			int distance = Math.abs((int) getCurrentChampion().getLocation().getX() - x)
					+ Math.abs((int) getCurrentChampion().getLocation().getY() - y);
			if (board[x][y] == null || distance > a.getCastRange()
					|| board[x][y] instanceof Cover && !(a instanceof DamagingAbility))
				return;
		}
		if (board[x][y] == null)
		...
		if (board[x][y] instanceof Champion) {
			...
			if (this.getSecondPlayer().getTeam().contains(this.getCurrentChampion())) {
				if (friendly && a instanceof DamagingAbility
						|| friendly && a instanceof CrowdControlAbility
								&& ((CrowdControlAbility) a).getEffect().getType() == EffectType.DEBUFF
						|| !friendly && a instanceof HealingAbility || !friendly && a instanceof CrowdControlAbility
								&& ((CrowdControlAbility) a).getEffect().getType() == EffectType.BUFF)
					return;
			} else {
				if (friendly && a instanceof DamagingAbility)
		...
		for (Damageable c : targets) {
			if (c.getCurrentHP() == 0) {
				if (c instanceof Champion)
					((Champion) c).animation.direction = "dead";
				if (c instanceof Cover)
					((Cover) c).animation.direction = "dead";
	...
	public static void main(String[] args) throws IOException {
		// Game a = new Game(new Player("a"), new Player("a"));
		// a.onRun();
		// System.out.print(a.getAvailableChampions().get(0));
