...
public class Game {
		...
		for (int i = 0; i < getCurrentChampion().getAttackRange(); i++) {
			...
			if (currx < 0 || currx >= BOARDHEIGHT || curry < 0 || curry >= BOARDWIDTH)
				...
				if (board[currx][curry] instanceof Cover) {
					...
					getCurrentChampion().animation.targets=new ArrayList<Damageable>();
					getCurrentChampion().animation.targets.add((Cover) board[currx][curry]);
					((Cover) board[currx][curry]).animation.attacked = true;
					((Cover) board[currx][curry]).animation.currentHp = curhp;
					((Cover) board[currx][curry]).animation.makeSound = true;
					...
					getCurrentChampion().animation.targets=new ArrayList<Damageable>();
					getCurrentChampion().animation.targets.add((Champion) board[currx][curry]);
	...
	public void castAbility(Ability a)
		...
		getCurrentChampion().animation.targets=targets;
		a.execute(targets);
	...
	public void useLeaderAbility() throws LeaderNotCurrentException, LeaderAbilityAlreadyUsedException {
		...
		ArrayList<Damageable> tmp = new ArrayList<Damageable>();
		for(int i=0;i<targets.size();i++)
		tmp.add(targets.get(i));
		
		getCurrentChampion().animation.targets= tmp;
